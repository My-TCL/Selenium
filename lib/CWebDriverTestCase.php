...
class CWebDriverTestCase extends PHPUnit_Framework_TestCase
{
    ...
    */
    ...
    public function open($url, $check_id = '')
    {
        ...
        if (!empty ($check_id)) {
            return $this->getElement("id", $check_id);
        } else {
    ...
    {
        $element = $this->getElement("id", $element_id);
        if (isset ($element)) {
    ...
    {
        $element = $this->getElement("id", $element_name);
        if (isset ($element)) {
            $length = mb_strlen($textToType);
            for ($i = 0; $i < $length; $i++) {
                //TODO: Check if this needs conversion or not
                $element->sendKeys(array(
                    mb_substr($textToType, $i, 1)
                ));
                sleep(0.1);
            }
        }
    ...
    {
        $element = $this->getElement("id", $select_id);
        $select = new WebDriverSelect($element);
        $select->deselectByVisibleText($option_text);
    }
    ...
    {
        try {
            $element = $this->webdriver->findElement(WebDriverBy::id($element_name));
        } catch (NoSuchElementException $ex) {
            try {
                $element = $this->webdriver->findElement(WebDriverBy::name($element_name));
            } catch (NoSuchElementException $ex) {
                $element = null;
            }
        }
     ...
     * @param type string
     * @param string $name
     * @param boolean $failIfNotExists
     *            whether should fail test if element not exists
    ...
    public function getElement($strategy, $name, $failIfNotExists = true)
    {
        ...
        do {
            try {
                $element = $this->webdriver->findElement(WebDriverBy::$strategy($name));
